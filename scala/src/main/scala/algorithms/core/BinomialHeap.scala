...
import icfp.algorithms.core.Heap
import icfp.algorithms.heaps.Heap
...
class BinomialHeap[A <% Ordered[A]](val aMin: A) extends Heap[A] {
  ...
  */
  def extractMin(): Option[A] = {
    ...
    while (c != null) {
      // this takes log(n) time as there are as many root nodes
      if (c.key < x.key) {
        x = c
        x_prev = c_prev
      }
  ...
  */
  def merge(H1: BinomialHeap[A], H2: BinomialHeap[A]): BHNode[A] = {
    ...
    while (p1 != null || p2 != null) {
      ...
      if (p1 == null) {
        if (h == null) h = p2 else p -> p2; p = p2; p2 = p2 ->
      } // if p1 is null or its degree is greater than p2's
      else if (p2 == null || p1.degree <= p2.degree) {
        if (h == null) h = p1 else p -> p1; p = p1; p1 = p1 ->
      } // if p2 is null or its degree is greater than p1's
      else {
        if (h == null) h = p2 else p -> p2; p = p2; p2 = p2 ->
      }
  ...
  def union(H1: BinomialHeap[A], H2: BinomialHeap[A]): BinomialHeap[A] = {
    ...
    while (next_x != null) {
      if ((x.degree != next_x.degree) || (next_x.sibling != null && next_x.sibling.degree == x.degree)) {
        ...
        // if  both x and next_x have the same degree ..AND x's key is lesser than next_x's
        x -> next_x.sibling // skip next_x by joining x to next_x's sibling
        link(next_x, x) // make next_x a child of x
      } else {
        // if  both x and next_x have the same degree ..AND x's key is greater than next_x's
        if (prev_x == null) {
          // if x is the head ... may next_X as the new head as x is going to move under next-x now
          H.head = next_x
        } else {
